...
PersistentGui.Name = "GlobalToggle"
PersistentGui.Parent = game:GetService("CoreGui")

local ToggleButton = Instance.new("TextButton")
ToggleButton.Parent = PersistentGui
ToggleButton.Position = UDim2.new(0, 10, 0.7, 0) -- Lowered on screen
ToggleButton.Size = UDim2.new(0, 120, 0, 50) -- Wider and taller for clear text
...
-- Gui Names to Toggle
local guiNames = {"comp hub", "ArbixTPGui"}
...
ToggleButton.MouseButton1Click:Connect(function()
    ...
    for _, guiName in ipairs(guiNames) do
        local gui = game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui"):FindFirstChild(guiName)
        if gui then
            gui.Enabled = not hidden
        end
        local coreGui = game:GetService("CoreGui"):FindFirstChild(guiName)
        if coreGui then
            coreGui.Enabled = not hidden
        end
    end
end)
...
local function DebugInfo(mode, content, value)
    ...
    if mode == "warn" then
        warn("[CompTP DEBUG]:", content, value or "")
    elseif mode == "print" then
        print("[CompTP DEBUG]:", content, value or "")
    else
        warn("[CompTP DEBUG]: Invalid debug type.")
    end
end
...
local oldGui = game.CoreGui:FindFirstChild("BodyVelocitySpeedUI")
if oldGui then
    oldGui:Destroy()
end

local gui = Instance.new("ScreenGui", game.CoreGui)
gui.Name = "BodyVelocitySpeedUI"
...
local function destroyAllBodyVelocitySpeedUI()
    -- Search common UI containers
    local containers = {
        game.CoreGui,
    ...
    for _, container in pairs(containers) do
        for _, gui in pairs(container:GetChildren()) do
            if gui.Name == "BodyVelocitySpeedUI" then
                gui:Destroy()
            end
        end
    end
...
onBtn.Size = UDim2.new(0, 90, 0, 40)
onBtn.Position = UDim2.new(0.05, 0, 0.3, 0)
onBtn.Text = "Speed ON"
...
offBtn.Size = UDim2.new(0, 90, 0, 40)
offBtn.Position = UDim2.new(0.55, 0, 0.3, 0)
offBtn.Text = "Speed OFF"
...
local function applySpeed()
    RunService:UnbindFromRenderStep("BodyVelocitySpeed")
    ...
    RunService:BindToRenderStep("BodyVelocitySpeed", Enum.RenderPriority.Input.Value, function()
        if humanoid.MoveDirection.Magnitude > 0 then
            velocity.Velocity = humanoid.MoveDirection.Unit * SPEED
        else
            velocity.Velocity = Vector3.zero
        end
    end)
...
TitleLabel.Position = UDim2.new(0, 15, 0, 0)
TitleLabel.Font = Enum.Font.GothamBlack
TitleLabel.Text = "Comp hub version 1.2"
TitleLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
TitleLabel.TextSize = 20
TitleLabel.TextXAlignment = Enum.TextXAlignment.Left
TitleLabel.TextTransparency = 1
...
SubmitButton.MouseButton1Click:Connect(function()
    ...
    if userInput == key then
        ...
        ScreenGui:Destroy()
        Blur:Destroy()
        print("[comp hub]: CompBlur destroyed")
